...
from sceptre.plan.actions import StackActions, StackGroupActions
...
class TestStackGroupActions(object):
    ...
    def setup_method(self, test_method):
        ...
        self.stack_group.stacks.append(self.stack)
        self.actions = StackGroupActions(self.stack_group)

        # Run the rest of the tests against a leaf stack_group
        self.stack_group._is_leaf = True

    def test_initialise_stack_group_actions(self):
        assert self.actions.stack_group == self.stack_group

    @patch("sceptre.plan.actions.StackGroupActions._build")
    @patch("sceptre.plan.actions.StackGroupActions._get_launch_dependencies")
    @patch("sceptre.plan.actions.StackGroupActions._get_initial_statuses")
    @patch("sceptre.plan.actions.StackGroupActions._get_threading_events")
    def test_launch_calls_build_with_correct_args(
        ...
        mock_get_launch_dependencies.return_value = sentinel.dependencies
    ...
    @patch("sceptre.plan.actions.StackGroupActions._build")
    @patch("sceptre.plan.actions.StackGroupActions._get_delete_dependencies")
    @patch("sceptre.plan.actions.StackGroupActions._get_threading_events")
    def test_launch_succeeds_with_empty_group(
            self, mock_get_threading_events,
            mock_get_delete_dependencies, mock_build
    ):

        self.actions.stack_group.stacks = {}
        mock_get_threading_events.return_value = sentinel.threading_events
    ...
    def test_delete_succeeds_with_empty_group(self):
        ...
        response = self.actions.delete()
        assert response == {}

    @patch("sceptre.plan.actions.StackActions.get_status")
    def test_describe_with_running_stack(self, mock_get_status):
        self.stack.name = "stack"
        mock_get_status.return_value = "status"
        response = self.actions.describe()
        assert response == {"stack": "status"}

    @patch("sceptre.plan.actions.StackActions.get_status")
    def test_describe_with_missing_stack(self, mock_get_status):
        self.stack.name = "stack"
        mock_get_status.side_effect = StackDoesNotExistError()
        response = self.actions.describe()
        assert response == {"stack": "PENDING"}

    @patch("sceptre.plan.actions.StackActions.describe_resources")
    def test_describe_resources_forms_response(self, mock_describe_resources):
        self.stack.name = "stack"
        mock_describe_resources.return_value = [
            {
                "LogicalResourceId": sentinel.logical_resource_id,
                "PhysicalResourceId": sentinel.physical_resource_id
            }
    ...
    @patch("sceptre.plan.actions.StackActions.describe_resources")
    def test_describe_resources_ignores_stack_does_not_exist_exception(
        self, mock_describe_resources
    ):
        self.stack.name = "stack"
        mock_describe_resources.side_effect = ClientError(
            {
                "Error": {
                    "Code": 500,
                    "Message": "stack does not exist"
    ...
    @patch("sceptre.plan.actions.StackActions.describe_resources")
    def test_describe_resources_raises_other_client_errors(
            self, mock_describe_resources
    ):
        self.stack.name = "stack"
        mock_describe_resources.side_effect = ClientError(
        ...
        with pytest.raises(ClientError):
            self.actions.describe_resources()

    @patch("sceptre.plan.actions.wait")
    @patch("sceptre.plan.actions.ThreadPoolExecutor")
    ...
    def test_manage_stack_build_with_failed_dependency(self):
        ...
        self.actions._manage_stack_build(
            mock_stack_2,
            sentinel.command,
            threading_events,
            stack_statuses,
    ...
    @patch("sceptre.plan.actions.threading.Event")
    def test_get_threading_events(self, mock_Event):
        self.stack.name = "stack"
        mock_Event.return_value = sentinel.event
        response = self.actions._get_threading_events()
        assert response == {
    ...
    def test_get_initial_statuses(self):
        self.stack.name = "stack"
        response = self.actions._get_initial_statuses()
        assert response == {
            "stack": StackStatus.PENDING
        }

    def test_get_launch_dependencies(self):
        self.stack.name = "dev/mock_stack"
        ...
        response = self.actions._get_launch_dependencies()
    ...
    def test_get_delete_dependencies(self, mock_get_launch_dependencies):
        ...
        dependencies = self.actions._get_delete_dependencies()
