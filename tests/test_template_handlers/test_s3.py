...
class TestS3(object):
    def test_get_template(self):
        ...
        connection_manager.call.return_value = {"Body": io.BytesIO(b"Stuff is working")}
        template_handler = S3(
            name="s3_handler",
        ...
        connection_manager.call.assert_called_once_with(
            service="s3",
            command="get_object",
            kwargs={"Bucket": "bucket", "Key": "folder/file.yaml"},
        )
    ...
    def test_template_handler(self):
        ...
        connection_manager.call.assert_called_once_with(
            service="s3",
            command="get_object",
            kwargs={"Bucket": "my-fancy-bucket", "Key": "account/vpc.yaml"},
        )
        assert result == b"Stuff is working"
    ...
    def test_handler_jinja_template(
        slef, mock_get_template, mock_render_jinja_template
    ):
