...
def set_template_path(context, stack_name, template_name):
    ...
    template_path = os.path.join(
        sceptre_context.project_path, sceptre_context.templates_path, template_name
    )
...
@when(
    'the user validates the template for stack "{stack_name}" with ignore dependencies'
)
def step_impl(context, stack_name):
    sceptre_context = SceptreContext(
...
def step_impl(context, filename):
    filepath = os.path.join(context.sceptre_dir, "templates", filename)
