...
@given('stack "{stack_name}" has change set "{change_set_name}" using "{filename}"')
...
def step_impl(context, stack_name):
    full_name = get_cloudformation_stack_name(context, stack_name)
    response = retry_boto_call(context.client.list_change_sets, StackName=full_name)
...
@when(
    'the user creates change set "{change_set_name}" for stack "{stack_name}" with ignore dependencies'
)
def step_impl(context, change_set_name, stack_name):
    sceptre_context = SceptreContext(
...
@when(
    'the user deletes change set "{change_set_name}" for stack "{stack_name}" with ignore dependencies'
)
def step_impl(context, change_set_name, stack_name):
    sceptre_context = SceptreContext(
        command_path=stack_name + ".yaml",
...
@when(
    'the user executes change set "{change_set_name}" for stack "{stack_name}" with ignore dependencies'
)
def step_impl(context, change_set_name, stack_name):
    sceptre_context = SceptreContext(
        command_path=stack_name + ".yaml",
...
@when(
    'the user describes change set "{change_set_name}" for stack "{stack_name}" with ignore dependencies'
)
def step_impl(context, change_set_name, stack_name):
    sceptre_context = SceptreContext(
        command_path=stack_name + ".yaml",
