Feature: Stack output resolver
  ...
  Scenario: launch a executor where stacks reference other stack outputs
    Given executor "6/1" does not exist
    When the user launches executor "6/1"
    Then all the stacks in executor "6/1" are in "CREATE_COMPLETE"
  ...
  Scenario: delete a stack referencing an output of existing stack
    ...
    When the user deletes executor "6/1"
    Then all the stacks in executor "6/1" do not exist
